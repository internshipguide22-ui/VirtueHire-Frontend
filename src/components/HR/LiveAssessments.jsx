...
import { Trash2, Lock, Unlock, AlertTriangle, CheckCircle2, ShieldAlert } from 'lucide-react';
import api from '../../services/api';
import './LiveAssessments.css';

const LiveAssessments = ({ hr, refreshTrigger }) => {
  ...
  const fetchLiveAssessments = async () => {
    ...
    try {
      ...
      const res = await api.get('/hrs/assessments/live', { withCredentials: true });
  ...
  const toggleLock = async (ast) => {
    ...
    try {
      await api.put(`/hrs/assessments/${ast.id}/lock?lock=${newLockStatus}`, {}, { withCredentials: true });
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await api.delete(`/hrs/assessments/${testToDelete.id}`, { withCredentials: true });
  ...
  if (loading) {
     return <div className="la-loading"><span className="spinner-border spinner-border-sm me-2"></span> Loading Assessments...</div>;
  }
    ...
    <div className="la-container">
      <div className="la-header">
        <div>
           ...
           <p>Control availability, monitoring configurations, and active test banks.</p>
        </div>
      </div>

      {error && <div className="la-alert la-alert-error"><AlertTriangle size={18} /> {error}</div>}
        ...
        {assessments.length === 0 ? (
          ...
          <div className="la-table-responsive">
            <table className="la-table">
              ...
              <tbody>
                {assessments.map((ast) => (
                  <tr key={ast.id} className={ast.isLocked ? "la-row-locked" : ""}>
                    <td className="la-fw-600">
                      <div className="la-test-info">
                        {ast.assessmentName}
                        {ast.isLocked && <span className="la-lock-tag"><Lock size={12} /> Locked</span>}
                      </div>
                    </td>
                    <td>
                      <span className="la-badge la-badge-gray">{ast.sectionCount} Sections</span>
                    </td>
                    ...
                    <td>
                      {ast.isLocked ? (
                        ...
                        <span className="la-badge la-badge-success">Active</span>
                      )}
                    </td>
                    <td>
                      <div className="la-action-btns">
                        ...
                        >
                           {ast.isLocked ? <Unlock size={18} /> : <Lock size={18} />}
                        </button>
                        <button 
        ...
        <div className="la-modal-overlay">
          <div className="la-modal-content">
            ...
            <div className="la-modal-body">
               ...
               <p>Are you sure you want to permanently delete <strong>{testToDelete?.assessmentName}</strong>? This cannot be undone.</p>
            </div>
            <div className="la-modal-footer">
               <button className="la-btn la-btn-outline" onClick={() => setShowDeleteModal(false)} disabled={isDeleting}>Cancel</button>
               <button className="la-btn la-btn-danger" onClick={confirmDelete} disabled={isDeleting}>
