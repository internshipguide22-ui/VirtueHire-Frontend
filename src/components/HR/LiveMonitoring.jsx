...
const LiveMonitoring = () => {
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            case 'Active': return <span className="lm-badge lm-badge-green">Active</span>;
            case 'Warning': return <span className="lm-badge lm-badge-yellow">Warning</span>;
            case 'Suspicious': return <span className="lm-badge lm-badge-red">Suspicious</span>;
            case 'Submitted': return <span className="lm-badge lm-badge-blue">Submitted</span>;
            default: return <span className="lm-badge">{status}</span>;
        }
    };
    ...
    return (
        <div className="lm-container">
            <div className="lm-header">
                ...
                <div className={`lm-live-indicator ${connected ? '' : 'lm-disconnected'}`}>
            ...
            <div className="lm-table-card">
                {candidateList.length === 0 ? (
                    <div className="lm-empty-state">
                        ...
                        <p>Real-time data will appear here when candidates start their assessments.</p>
                    </div>
                ) : (
                    <div className="lm-table-responsive">
                        <table className="lm-table">
                            ...
                            <tbody>
                                {candidateList.map((c) => (
                                    <tr key={c.candidateId} className={getViolationColorClass(c.violationCount)}>
                                        <td>
                                            <div className="lm-candidate-info">
                                                <div className="lm-avatar">{c.candidateName.charAt(0)}</div>
                                                <div>
                                        ...
                                        <td><span className="lm-badge lm-badge-blue">{c.section || 'N/A'}</span></td>
                                        ...
                                        <td>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontWeight: '700' }}>
                                                {c.violationCount >= 3 && <AlertCircle size={16} className="lm-alert-badge" />}
                                                <span style={{ color: c.violationCount >= 3 ? '#ef4444' : (c.violationCount > 0 ? '#f59e0b' : 'inherit') }}>
                                                    {c.violationCount} / 3
                                                </span>
                                            </div>
                                        </td>
