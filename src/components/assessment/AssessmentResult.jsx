...
const AssessmentResult = () => {
  ...
  useEffect(() => {
    ...
    const fetchLatestResult = async () => {
      ...
      setLoading(true);
      try {
        const res = await api.get(`/assessment/results/${subj}`, { withCredentials: true });
        ...
        const statusRes = await api.get(`/assessment/status/${subj}`, { withCredentials: true });
  ...
  const handleNextLevel = () => {
    ...
    } else if (passed && subject && level !== null) {
      navigate(`/assessment/${subject}/${level + 1}`);
    } else {
      // If they failed and it's auto summitted, just take them home
